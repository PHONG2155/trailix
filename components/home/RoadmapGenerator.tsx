...
// Thêm import hook từ react-router-dom để điều hướng mượt mà giống Navbar
...
const TerminalLoader = () => {
  ...
  const logs = [
    ...
    "Fetching Prompt Engineering modules...",
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button 
        ...
        className={`w-full bg-white border text-left rounded-xl pl-12 pr-4 py-3.5 font-bold flex items-center justify-between transition-all shadow-sm outline-none ${
          isOpen ? 'border-brand-red ring-2 ring-brand-red/20 shadow-md' : 'border-gray-200 hover:border-brand-red/60 hover:shadow-md'
        }`}
      >
          ...
          >
            {options.map((option) => (
              <div
                ...
                className={`px-4 py-2.5 cursor-pointer flex items-center justify-between group transition-all rounded-lg mb-0.5 ${
                  value === option ? 'bg-red-50 text-brand-red' : 'hover:bg-gray-50 text-gray-700'
                }`}
              >
...
export const RoadmapGenerator = () => {
  ...
  // Khai báo hook điều hướng giống Navbar
  const navigate = useNavigate();
  const location = useLocation();

  const roleOptions = [...Object.values(JobRole), 'Khác'];
  const levelOptions = [...Object.values(ExperienceLevel), 'Khác'];

  const handleGenerate = async () => {
    const finalRole = roleSelection === 'Khác' ? customRole : roleSelection;
    const finalLevel = levelSelection === 'Khác' ? customLevel : levelSelection;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black font-display text-brand-black mb-6">
            Thiết Kế Lộ Trình <span className="text-brand-red">Thông Minh</span>
          </h2>
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Hệ thống AI sẽ phân tích vị trí công việc của bạn để tạo ra giáo trình học tập cá nhân hóa chính xác nhất.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white rounded-3xl p-6 md:p-10 shadow-2xl shadow-gray-200/50 border border-gray-100">
          ...
          <div className="lg:col-span-4 space-y-8 flex flex-col justify-center">
            <div>
               <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 ml-1">Vị trí của bạn</label>
               <ModernSelect 
                 options={roleOptions}
                 value={roleSelection}
            ...
            <div>
               <label className="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 ml-1">Cấp độ sử dụng AI hiện tại</label>
               <ModernSelect 
                 ...
                 {levelSelection === 'Khác' && (
                    ...
                    >
                      <input 
                         ...
                         placeholder="Nhập cấp độ cụ thể..."
                         className="w-full bg-white border border-gray-200 rounded-xl px-4 py-3.5 outline-none focus:border-brand-red focus:ring-2 focus:ring-brand-red/10 transition-all text-gray-700 font-medium text-sm shadow-sm"
                         autoFocus
                      />
                    </motion.div>
            ...
            <motion.button 
              ...
              className={`w-full py-5 rounded-xl font-black uppercase tracking-wide transition-all flex items-center justify-center gap-3 shadow-lg shadow-brand-red/20 ${
                !isReady ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 
            ...
            >
              {loading ? (
                ...
                <>
                  <Zap className="w-5 h-5 fill-current" /> Tạo lộ trình ngay
                </>
              )}
            </motion.button>
          </div>
             ...
             {loading ? (
               ...
               <div className="h-full overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                  <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900 font-display">Lộ trình cá nhân hoá</h3>
                  ...
                  >
                    <div>
                      <p className="text-gray-600 mb-4">Bạn muốn được hướng dẫn chi tiết theo lộ trình này?</p>
                      <motion.button 
                      ...
                      >
                        <span>Đăng ký tư vấn chuyên sâu</span>
               ...
               <div className="flex flex-col items-center justify-center h-full text-center p-8 opacity-60">
                 ...
                 <h3 className="text-xl font-bold text-gray-900 mb-1">Chờ dữ liệu đầu vào</h3>
                 <p className="text-sm text-gray-500">Hãy chọn vị trí và cấp độ để khởi động AI</p>
               </div>
             )}
          </div>
        </div>
