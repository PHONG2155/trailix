...
export const AiVideo = () => {
  ...
  // Data mapping
  const valuesData = t('detail.ai-video.values.items', { returnObjects: true }) as any[];
  const modulesData = t('detail.ai-video.modules.items', { returnObjects: true }) as any[];
  const targetTags = t('detail.ai-video.target.tags', { returnObjects: true }) as string[];

  // Icons for values
  const ValueIcons = [Clapperboard, Video, MonitorPlay, Zap];
      ...
      <div className="relative z-10">
        ...
        <section className="min-h-screen flex flex-col justify-center items-center relative px-4">
          ...
          >
            ...
            <span className="text-sm font-bold text-red-700 tracking-wide uppercase">{t('detail.ai-video.hero.badge')}</span>
          </motion.div>
          ...
          <div className="max-w-screen-2xl mx-auto text-center space-y-6 px-4">
            ...
            >
              {t('detail.ai-video.hero.title.main')}
              <span className="text-red-700 block mt-2 md:mt-4">
                {t('detail.ai-video.hero.title.sub')}
              </span>
            </motion.h1>
            ...
            >
              <span dangerouslySetInnerHTML={{ __html: t('detail.ai-video.hero.subtitle', { interpolation: { escapeValue: false } }) }} />
            </motion.p>
            ...
            >
              <div className="text-sm font-bold text-slate-500 flex items-center justify-center gap-2 px-8 py-3 rounded-full bg-slate-50 border border-slate-200 inline-flex hover:border-red-200 hover:bg-white transition-colors cursor-default">
                ...
                {t('detail.ai-video.hero.duration')}
              </div>
            </motion.div>
        ...
        <section className="py-24 bg-white relative">
          ...
          >
            <h3 className="text-3xl md:text-4xl font-black leading-tight text-slate-900">
              {t('detail.ai-video.message.text')} <br />
              <span className="text-red-600">
                {t('detail.ai-video.message.highlight')}
              </span>
            </h3>
          </motion.div>
        </section>
        ...
        <section className="py-24 px-4 bg-slate-50">
          <div className="max-w-6xl mx-auto">
            ...
            >
              <h2 className="text-sm font-bold text-red-600 uppercase tracking-widest mb-2">{t('detail.ai-video.values.badge')}</h2>
              <h3 className="text-4xl font-black text-slate-900">{t('detail.ai-video.values.title')}</h3>
            </motion.div>
        ...
        <section className="py-24 px-4 bg-white relative">
          <div className="max-w-5xl mx-auto">
            <motion.div
              ...
              className="text-center mb-16"
            >
              <h2 className="text-4xl font-black text-slate-900 mb-2">{t('detail.ai-video.modules.title')}</h2>
              <p className="text-slate-500 text-lg">{t('detail.ai-video.modules.subtitle')}</p>
            </motion.div>
        ...
        <section className="py-24 bg-slate-50 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 relative z-10 text-center">
            <motion.div
              ...
              className="mb-16"
            >
              <h2 className="text-3xl font-black mb-8 text-slate-900">{t('detail.ai-video.target.title')}</h2>
            ...
            >
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-5 justify-center items-center">
                ...
                <motion.button
                  ...
                  className="w-full md:w-[280px] h-16 bg-red-700 text-white rounded-full font-bold text-lg shadow-lg shadow-red-700/20 flex justify-center items-center gap-3"
                >
                  {t('detail.ai-video.cta.register')}
                  <ArrowRight className="w-5 h-5" />
                </motion.button>
